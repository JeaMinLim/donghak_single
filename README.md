...
동학은 DART(금융감독원 전자공시시스템)에 올라와 있는 정기공시에서 필요한 정보를 추출합니다. 
AI에게 특정 기업의 정보를 물어보면 부 정확한 테이터 또는 과거의 어느 시점의 데이터를 기반으로 알려줍니다. 가끔 잘못된 정보나 너무 오래된 정보를 기반으로 알려주는 문제가 있습니다. 
동학은 이런 문제를 해결하고자 정기공시의 내용 중 요약재무정보와 매출 정보를 추출해서 Markdown으로 만드는 기능을 수행합니다. 변환된 Markdown은 AI에게 입력하는 데이터로서 활용할 수 있습니다. 
